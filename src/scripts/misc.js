...
export function createAlert(i18n, type, message, boldMessage, textWithBold, textWithBoldMessage, timeout = 0, extraMessage, MIN_PASS_LENGTH, text, dynamicValue, dynamicValueMessage) {
    // DOM Cache
    ...
    // Message
    domAlert.innerHTML = boldMessage ? `<b>${i18n.t(boldMessage)}</b><br/>${i18n.t(message)}` : textWithBold ? `<b>${i18n.t(textWithBoldMessage)}</b> ${i18n.t(textWithBold)}<br/>${i18n.t(message)}` : extraMessage ? `${i18n.t(message)}<br/>${i18n.t(extraMessage)} <b>${MIN_PASS_LENGTH} ${i18n.t(text)}</b>` : dynamicValue ? `<b>${i18n.t(dynamicValueMessage)}</b><br/>${i18n.t(message, { name: dynamicValue })}` : i18n.t(message);
    domAlert.destroy = () => {
