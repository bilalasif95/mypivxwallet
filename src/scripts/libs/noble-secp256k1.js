...
function nobleSecp256k1(global, factory) {
	...
	typeof exports === 'object' && typeof module !== 'undefined' ? factory(exports, crypto) :
		typeof define === 'function' && this.define.amd ? this.define(['exports', 'crypto'], factory) :
			(global = typeof this.globalThis !== 'undefined' ? this.globalThis : global || this.self, factory(global.nobleSecp256k1 = {}, global.require$$0));
	// eslint-disable-next-line no-unused-expressions
	...
	(function (exports) {
		...
		exports.utils = {
			...
			randomBytes: (bytesLength = 32) => {
				if (typeof this.self == 'object' && 'crypto' in this.self) {
					return this.self.crypto.getRandomValues(new Uint8Array(bytesLength));
				}
			...
			sha256: async (message) => {
				if (typeof this.self == 'object' && 'crypto' in this.self) {
					const buffer = await this.self.crypto.subtle.digest('SHA-256', message.buffer);
			...
			hmacSha256: async (key, ...messages) => {
				if (typeof this.self == 'object' && 'crypto' in this.self) {
					const ckey = await this.self.crypto.subtle.importKey('raw', key, { name: 'HMAC', hash: { name: 'SHA-256' } }, false, ['sign']);
					const message = concatBytes(...messages);
					const buffer = await this.self.crypto.subtle.sign('HMAC', ckey, message);
					return new Uint8Array(buffer);
				}
				else if (typeof process === 'object' && 'node' in process.versions) {
					const { createHmac } = require$$0__default['default'];
	...
	const index = /*@__PURE__*/getDefaultExportFromCjs(secp256k1);
